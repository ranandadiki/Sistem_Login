...
class User extends CI_Controller
{
    ...
    public function changePassword()
    {
        ...
        $this->form_validation->set_rules('newpassword2', 'Confirmation', 'required|trim|min_length[3]|matches[newpassword1];');
        ...
        if ($this->form_validation->run() == false) {
            ...
            if (!password_verify($current_password, $data['user']['password'])) {
                ...
                if ($current_password == $new_password) {
                    $this->session->set_flashdata('message', '<div class="alert alert-danger" role="alert">New Password can not same!</div>');
                    redirect('user/changepassword');
                } else {
                    //password input ok
